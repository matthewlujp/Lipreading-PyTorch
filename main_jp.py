...
def load_pretrained_bases(model: nn.Module, pretrained_model_filepath: str):
    if cuda.is_available():
        pretrained_dict = torch.load(pretrained_model_filepath)
    else:
        pretrained_dict = torch.load(pretrained_model_filepath, map_location=lambda storage, loc: storage)
    ...
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k.startswith("frontend.") or k.startswith("resnet.")}
    model_dict.update(pretrained_dict) 
    model.load_state_dict(model_dict)
...
if __name__ == '__main__':
    ...
    if args.checkpoint_file is None:
        ...
        if options["general"]["train_target"] == 'backend':
            ...
            load_pretrained_bases(model, options["training"]["base_model_path"])
